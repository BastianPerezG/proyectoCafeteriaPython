...
class Boletas:
    ...
    def mostrar_bol_usu(id):
        sql = f"SELECT u.usu_nom,b.bol_cod, b.bol_iva, b.bol_total, v.ven_descrip FROM boletas b JOIN ventas v ON (b.ven_cod = v.ven_cod) JOIN usuarios u ON (v.usu_cod = u.usu_cod) WHERE u.usu_cod = {id};"
        col1 = "USERNAME"
        col2 = "ID"
        col3 = "IVA"
        col4 = "TOTAL"
    ...
    def mostrar_bol_cli(id):
        sql = f"SELECT Concat(p.per_nombre, p.per_app),b.bol_cod, b.bol_iva, b.bol_total, v.ven_descrip FROM boletas b JOIN ventas v ON (b.ven_cod = v.ven_cod) JOIN clientes c ON (v.cli_cod = c.cli_cod) JOIN personas p ON (c.per_run = p.per_run) WHERE c.cli_cod = {id};"
